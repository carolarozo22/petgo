package com.peigo.wallet.ms.boilerplate.constants;

public class SpringDoc {

    /**
     * Variables de configuración
     */
    public static final String CONFIG_TITLE = "boilerplate";
    public static final String CONFIG_DESCRIPTION = "Estructura base del proyecto";
    public static final String CONFIG_VERSION = "0.0.1";
    public static final String CONFIG_TERMS = "Terminos";
    public static final String CONFIG_EMAIL = "diana.rozo@globant.com";
    public static final String CONFIG_CONTACT_URL = "https://www.globant.com";
    public static final String CONFIG_CONTACT_NAME = "Peigo";
    public static final String CONFIG_LICENSE_NAME = "GPL";
    public static final String CONFIG_LICENSE_URL = "http://umh2820.edu.umh.es/wp-content/uploads/sites/885/2016/02/GNU-General-Public-License.pdf";

    /**
     * Variables del controlador
     */
    public static final String CONTROLLER_USER_CREATE_USER_PATH = "/createUser";
    public static final String CONTROLLER_USER_CREATE_USER_SUMMARY = "Metodo que registra usuario";
    public static final String CONTROLLER_USER_DESCRIPTION_200 = "Respuesta exitosa";
    public static final String CONTROLLER_USER_DESCRIPTION_404 = "Usuario no encontrado";
    public static final String CONTROLLER_USER_TAG_NAME = "Controlador de usuarios";
    public static final String CONTROLLER_USER_TAG_DESCRIPTION = "Procesar información de usuarios";
    public static final String CONTROLLER_USER_FIND_USER_SUMMARY = "Metodo que busca usuario";

    /**
     * Variables DTO Usuarios
     */
    public static final String DTO_USER_SCHEMA_NAME = "DTO usuario";
    public static final String DTO_USER_SCHEMA_DESCRIPTION = "DTO para procesar la información del usuario";
    public static final String DTO_USER_MESSAGE_NAME = "Nombre vacío";
    public static final String DTO_USER_SCHEMA_NAME_DESCRIPTION = "Nombre del usuario";
    public static final String DTO_USER_SCHEMA_NAME_EXAMPLE = "Erick";
    public static final String DTO_USER_MESSAGE_EMAIL = "Formato de correo invalido";
    public static final String DTO_USER_MESSAGE_AGE = "Persona no cumple con la edad minima requerida";
    public static final String DTO_USER_NAME = "nombre";
    public static final String DTO_USER_EMAIL = "email";
    public static final String DTO_USER_AGE = "edad";

}
